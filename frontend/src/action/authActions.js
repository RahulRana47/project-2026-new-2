...
import { getMe, loginUser, firebaseAuthUser } from "../services/api";
...
export const loginWithFirebase = (idToken, payload = {}) => async (dispatch) => {
  try {
    dispatch({ type: LOGIN_REQUEST });

    const res = await firebaseAuthUser({ idToken, ...payload });
    ...
    if (res?.token) {
      ...
      dispatch({
        type: LOGIN_FAIL,
        payload: res?.message || "Firebase authentication failed",
      });
    }
  } catch (err) {
    dispatch({
      type: LOGIN_FAIL,
      payload: err?.message || "Firebase authentication failed",
    });
  }
};
